...
void EventRecordHIP(Event* event, const void* context, const char* err_msg) {
  auto* wrapper = static_cast<HipEventWrapper*>(event->event_.get());
  {
    ...
    //
    // No further changes to hip_event_ and hip_stream_ after transitioning
    // from INITIALIZED
...
void EventFinishHIP(const Event* event) {
  ...
  if (wrapper->status_ == EventStatus::EVENT_SCHEDULED) {
    ...
    HIPGuard g(wrapper->device_id_);
    auto cudaResult = hipEventSynchronize(wrapper->hip_event_);
    if (cudaResult == hipSuccess) {
      wrapper->status_ = EventStatus::EVENT_SUCCESS;
    } else {
      const auto& err_msg = hipGetErrorString(cudaResult);
...
EventStatus EventQueryHIP(const Event* event) {
  auto* wrapper = static_cast<HipEventWrapper*>(event->event_.get());
  if (wrapper->status_ == EventStatus::EVENT_SCHEDULED) {
    auto cudaResult = hipEventQuery(wrapper->hip_event_);
    if (cudaResult == hipSuccess) {
      wrapper->status_ = EventStatus::EVENT_SUCCESS;
    } else if (cudaResult != hipErrorNotReady) {
      const auto& err_msg = hipGetErrorString(cudaResult);
...
const std::string& EventErrorMessageHIP(const Event* event) {
  auto* wrapper = static_cast<HipEventWrapper*>(event->event_.get());
  // supposed to be called after EventQueryHIP to update status first
