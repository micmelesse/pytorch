...
__global__ void ComputeFusedParamHIPKernel<float>(
    ...
    float* alpha,
    float* beta) {
  const int c = blockIdx.x * CAFFE_HIP_NUM_THREADS + threadIdx.x;
  if (c < C) {
...
__global__ void ComputeScaleBiasGradientsAndFusedParamsNCHWHIPKernel(
    const int N,
    const int C,
    const int HxW,
    const T* dY,
...
template <typename T>
__global__ void ComputeScaleGradientAndFusedParamsNHWCHIPKernel(
    const int C,
    const T mean_scale,
    const T* dYxX,
    const T* dbias,
...
template <typename T>
__global__ void ComputeXGradientNCHWHIPKernel(
    const int C,
    const int M,
    const int HxW,
    const T* dY,
  ...
  const int c = nc % C;
  const int x = blockIdx.x % M * CAFFE_HIP_NUM_THREADS + threadIdx.x;
  if (x < HxW) {
...
template <typename T>
__global__ void ComputeXGradientNHWCHIPKernel(
    const int C,
    const int HxW,
    const T* dY,
    const T* X,
...
void SpatialBNOp<HIPContext>::ComputeFusedParam(
    ...
    T* alpha,
    T* beta) {
  const int M = math::DivUp(C, CAFFE_HIP_NUM_THREADS);
  ComputeFusedParamHIPKernel<T>
...
void SpatialBNOp<HIPContext>::ComputeBatchMoments(
  ...
  ComputeBatchMomentsHIPKernel<T>
      <<<M, CAFFE_HIP_NUM_THREADS, 0, context_.hip_stream()>>>(
...
void SpatialBNOp<HIPContext>::ComputeRunningMomentsAndFusedParam(
    ...
    T* beta) {
  const int M = math::DivUp(C, CAFFE_HIP_NUM_THREADS);
  ComputeRunningMomentsAndFusedParamHIPKernel<T>
      <<<M, CAFFE_HIP_NUM_THREADS, 0, context_.hip_stream()>>>(
          C,
          reduce_size,
...
void SpatialBNGradientOp<HIPContext>::
  ...
  const T mean_scale = T(1) / static_cast<T>(N * HxW);
  ComputeMultiBatchScaleBiasGradientsAndFusedParamsHIPKernel<T>
      <<<M, CAFFE_HIP_NUM_THREADS, 0, context_.hip_stream()>>>(
          C,
          batch_scale,
          mean_scale,
...
void SpatialBNGradientOp<HIPContext>::ComputeScaleBiasGradientsAndFusedParams(
  ...
  if (order_ == StorageOrder::NCHW) {
    DISPATCH_REDUCE_KERNEL_BY_2D_BLOCK_WITH_TYPE_1(
        HxW,
        ComputeScaleBiasGradientsAndFusedParamsNCHWHIPKernel,
        T,
        C,
        context_.hip_stream(),
        N,
    ...
    ReinitializeTensor(&ones_, N * HxW, at::dtype<T>().device(HIP));
    math::Set<T, HIPContext>(
        N * HxW, T(1), ones_.template mutable_data<T>(), &context_);
    const T* ones_data = ones_.template data<T>();
    math::Mul<T, HIPContext>(N * C * HxW, dY, X, scratch, &context_);
    math::Gemm<T, HIPContext>(
        CblasTrans,
        CblasNoTrans,
        C,
        1,
    ...
    math::Gemm<T, HIPContext>(
        ...
        &context_);
    const int M = math::DivUp(C, CAFFE_HIP_NUM_THREADS);
    ComputeScaleGradientAndFusedParamsNHWCHIPKernel<T>
        <<<M, CAFFE_HIP_NUM_THREADS, 0, context_.hip_stream()>>>(
            C,
...
void SpatialBNGradientOp<HIPContext>::ComputeXGradient(
    ...
    T* dX) {
  if (order_ == StorageOrder::NCHW) {
    const int M = math::DivUp(HxW, CAFFE_HIP_NUM_THREADS);
    ComputeXGradientNCHWHIPKernel<T>
        <<<N * C * M, CAFFE_HIP_NUM_THREADS, 0, context_.hip_stream()>>>(
            C, M, HxW, dY, X, alpha, beta, gamma, dX);
  } else {
    const int M = math::DivUp(C, CAFFE_HIP_NUM_THREADS);
    ComputeXGradientNHWCHIPKernel<T>
        <<<dim3(N * HxW, M),
           CAFFE_HIP_NUM_THREADS,
           0,
           context_.hip_stream()>>>(C, HxW, dY, X, alpha, beta, gamma, dX);
  }
}
