...
static bool gather_impl_hip(
    Operator<HIPContext>* op,
    int dataIdx,
    int indicesIdx,
    int outputIdx,
    int axis,
  ...
  CAFFE_ENFORCE(!match_outer, "match_outer=true is currently NOT supported for HIP");
  ...
  if (N > 0) {
    BatchGatherKernel<<<
        ::min(outer_dims_product, CAFFE_MAXIMUM_NUM_BLOCKS),
        ::min(N * block_size, CAFFE_HIP_NUM_THREADS),
        0,
        op->getContext()->hip_stream()>>>(
        src_base,
        out,
        idxs,
        outer_dims_product,
