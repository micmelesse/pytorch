...
inline __device__ randFactor<at::Half, float, STOCHASTIC>::randFactor(
    ulong2 seed,
    int thread_id) {
  hiprand_init(seed.x, thread_id, seed.y, &state);
}
...
#if (                         \
    (HIP_VERSION < 10000) || \
    (defined(__HIP_ARCH__) && (__HIP_ARCH__ < 700)))
...
__global__ void rowwise_sparse_adagrad_fused_length_sum_gradient_kernel(
  ...
  if (ExactBlock) {
    ...
    typedef hipcub::BlockReduce<float, REDUCE_SIZE> BlockReduce;
