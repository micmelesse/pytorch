...
struct DropoutState {
  ...
  void lock() {
    ...
    mutex.lock();
    if (event) {
      event->block(hip::getCurrentHIPStreamMasqueradingAsCUDA());
    }
  }
...
Tensor try_get_weight_buf(
      ...
      miopenRNNMode_t mode, int64_t hidden_size, int64_t proj_size, int64_t num_layers, bool bidirectional) {
