...
// Ensures we only call hipGetDeviceCount only once.
 ...
 * But jit kernels don't use hiprand, they use a custom "Philox" class (see
...
c10::intrusive_ptr<c10::TensorImpl> CUDAGeneratorImpl::get_state() const {
  ...
  // The following line is just here for BC reason. sizeof hiprandStateMtgp32_t is 4120.
  // It used to be static const size_t states_size = MAX_NUM_BLOCKS * sizeof(hiprandStateMtgp32_t);
  // MAX_NUM_BLOCKS was 200 and sizeof(hiprandStateMtgp32_t) is 4120. Hardcoding these numbers here
 ...
 * for philox is never smaller than the number of hiprand() calls. Increment
 * value > the number of hiprand() calls won't harm but anything less would mean
