...
void createCublasHandle(rocblas_handle *handle) {
  TORCH_CUDABLAS_CHECK(rocblas_create_handle(handle));
}

void destroyCublasHandle(rocblas_handle handle) {
...
using CuBlasPoolType = DeviceThreadHandlePool<rocblas_handle, createCublasHandle, destroyCublasHandle>;
...
rocblas_handle getCurrentCUDABlasHandle() {
  ...
  TORCH_CUDABLAS_CHECK(rocblas_set_stream(handle, stream));
  ...
  if (!NoTF32Guard::should_disable_tf32() && at::globalContext().allowTF32CuBLAS()) {
    TORCH_CUDABLAS_CHECK(rocblas_set_math_mode(handle, CUBLAS_TF32_TENSOR_OP_MATH));
  } else {
