...
public:
  PointerModeGuard(rocblas_handle handle, rocblas_pointer_mode mode) :
      handle(handle) {
    TORCH_CUDABLAS_CHECK(rocblas_get_pointer_mode(handle, &previous_mode));
    TORCH_CUDABLAS_CHECK(rocblas_set_pointer_mode(handle, mode));
  }

  ~PointerModeGuard() {
    rocblas_set_pointer_mode(handle, previous_mode);
  }

private:
  rocblas_handle handle;
  rocblas_pointer_mode previous_mode;
};
...
#define CUDABLAS_DOT_ARGTYPES(Dtype)                                      \
  rocblas_handle handle, int n, const Dtype *x, int incx, const Dtype *y, \
