...
struct TORCH_CUDA_CPP_API CUDAFuture : at::ivalue::Future {
 public:
  CUDAFuture(at::TypePtr type) : at::ivalue::Future(std::move(type)) {
    ...
    // in HIPGuardMasqueradingAsCUDA.
    currentDevice_ = c10::hip::current_device();
  }
 ...
 protected:
  ...
  void postMarkCompletedHook(const at::IValue& value) override {
    ...
    std::vector<bool> isCudaDeviceUsed(c10::hip::device_count(), false);
    ...
    for (c10::DeviceIndex idx = 0; idx < isCudaDeviceUsed.size(); idx++) {
      if (isCudaDeviceUsed[idx]) {
        ...
        cudaEvent.record(at::hip::getCurrentHIPStreamMasqueradingAsCUDA(idx));
    ...
    return [this, callback{std::move(callback)}]() {
      ...
      for (at::cuda::CUDAEvent& cudaEvent : cudaEvents_) {
        ...
        at::hip::HIPStreamMasqueradingAsCUDA stream =
            at::hip::getStreamFromPoolMasqueradingAsCUDA(/*isHighPriority=*/false, idx);
      ...
      for (const at::DataPtr& data_ptr : dataPtrs_) {
        if (data_ptr.device().is_cuda()) {
          c10::hip::HIPCachingAllocatorMasqueradingAsCUDA::recordStreamMasqueradingAsCUDA(
              data_ptr, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(data_ptr.device().index()));
        }
      }

      c10::hip::HIPGuardMasqueradingAsCUDA deviceGuard(currentDevice_);
  ...
  void postWaitHook(const at::IValue& value) override {
    for (at::cuda::CUDAEvent& cudaEvent : cudaEvents_) {
      cudaEvent.block(
          at::hip::getCurrentHIPStreamMasqueradingAsCUDA(cudaEvent.device_index()));
    }
