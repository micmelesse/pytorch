...
CUDAGraph::CUDAGraph()
  ...
  : capture_stream_(at::hip::getCurrentHIPStreamMasqueradingAsCUDA()) {
...
void CUDAGraph::capture_begin() {
  ...
  TORCH_CHECK(stream != at::hip::getDefaultHIPStreamMasqueradingAsCUDA(),
...
void CUDAGraph::replay() {
...

  {
    ...
    AT_CUDA_CHECK(cudaGraphLaunch(graph_exec_, at::hip::getCurrentHIPStreamMasqueradingAsCUDA()));
  }
#else
  TORCH_CHECK(false, "CUDA graphs may only be used in Pytorch built with CUDA >= 11.0");
#endif
...
void CUDAGraph::reset() {
  ...
  if (has_graph_) {
    C10_HIP_CHECK_WARN(cudaGraphDestroy(graph_));
  }
  if (has_graph_exec_) {
    C10_HIP_CHECK_WARN(cudaGraphExecDestroy(graph_exec_));
  }
#else
  TORCH_CHECK(false, "CUDA graphs may only be used in Pytorch built with CUDA >= 11.0");
#endif
