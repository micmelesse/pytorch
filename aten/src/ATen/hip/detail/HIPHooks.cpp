...
bool CUDAHooks::isPinnedPtr(void* data) const {
  ...
  // hipPointerGetAttributes grabs context on the current device, so we set
...
bool CUDAHooks::hasPrimaryContext(int64_t device_index) const {
  ...
  // In standalone tests of hipDevicePrimaryCtxGetState, I've seen the "active" argument end up with weird
  // (garbage-looking nonzero) values when the context is not active, unless I initialize it to zero.
  int ctx_is_active = 0;
  AT_CUDA_DRIVER_CHECK(CUDAHooks::nvrtc().hipDevicePrimaryCtxGetState(device_index, &ctx_flags, &ctx_is_active));
