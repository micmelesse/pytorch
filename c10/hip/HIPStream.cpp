...
struct LeakyStreamInternals {
  ...
  ~LeakyStreamInternals() {
    ...
    // (since we never shrink the corresponding vectors). At this point the HIP
    // runtime might be already destroyed and invoking hipStreamDestroy leads
    // to a crash. It's likely an issue in HIP, but to be safe - let's just
...
// Non-default streams
// Note: the number of HIP devices is determined at run time,
...
static StreamId HIPStream_getStreamId(const LeakyStreamInternals* ptr) {
...
static void initDeviceStreamState(DeviceIndex device_index) {
  ...
  // with it.
  HIPGuard device_guard{device_index};
  ...
  for (auto i = decltype(kStreamsPerPool){0}; i < kStreamsPerPool; ++i) {
    ...
    C10_HIP_CHECK(hipStreamCreateWithPriority(
...
static void initHIPStreamsOnce() {
...
// Note: Streams are returned round-robin (see note in HIPStream.h)
  ...
  switch (st) {
    case StreamIdType::DEFAULT:
      TORCH_INTERNAL_ASSERT(
          ...
          " official API like c10::hip::getStreamFromPool() to get a new stream.");
...
HIPStream HIPStream_fromInternals(const LeakyStreamInternals* ptr) {
  return HIPStream(
      HIPStream::UNCHECKED,
      Stream(
          Stream::UNSAFE,
          c10::Device(DeviceType::HIP, ptr->device_index),
          HIPStream_getStreamId(ptr)));
}

} // anonymous namespace

hipStream_t HIPStream::stream() const {
  auto ptr = HIPStream_internals(*this);
  AT_ASSERT(ptr);
  return ptr->stream;
}
...
HIPStream getStreamFromPool(
  ...
  if (isHighPriority) {
    ...
    return HIPStream_fromInternals(&high_priority_streams[device_index][idx]);
  }

  const auto idx = get_idx(low_priority_counters[device_index]);
  return HIPStream_fromInternals(&low_priority_streams[device_index][idx]);
}

HIPStream getDefaultHIPStream(DeviceIndex device_index) {
  initHIPStreamsOnce();
  if (device_index == -1) {
    device_index = current_device();
  }
  check_gpu(device_index);
  return HIPStream_fromInternals(&default_streams[device_index]);
}
HIPStream getCurrentHIPStream(DeviceIndex device_index) {
  initHIPStreamsOnce();
  if (device_index == -1) {
    device_index = current_device();
  }
  check_gpu(device_index);
  return HIPStream_fromInternals(current_streams[device_index]);
}

void setCurrentHIPStream(HIPStream stream) {
  initHIPStreamsOnce();
  auto ptr = HIPStream_internals(stream);
  AT_ASSERT(ptr);
  current_streams[ptr->device_index] = ptr;
}

std::ostream& operator<<(std::ostream& stream, const HIPStream& s) {
