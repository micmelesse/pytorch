...
C10_DEFINE_REGISTRY(FreeHipMemoryCallbacksRegistry, FreeMemoryCallback);

namespace hip {
namespace HIPCachingAllocator {
...
using stream_set = std::unordered_set<hip::HIPStream>;
...
void update_stat(Stat& stat, int64_t amount) {
  ...
  TORCH_INTERNAL_ASSERT(stat.current >= 0, "Negative tracked stat in HIP allocator (likely logic error).");
...
struct Block {
  ...
  Block(int device, hipStream_t stream, size_t size, BlockPool* pool, void* ptr) :
  ...
  Block(int device, hipStream_t stream, size_t size) :
...
struct AllocParams {
  AllocParams(int device, size_t size, hipStream_t stream, BlockPool* pool, size_t alloc_size,
              DeviceStats& stats) :
  ...
  hipStream_t stream() { return search_key.stream; }
...
class DeviceCachingAllocator {
 ...
 public:
  ...
  Block* malloc(int device, size_t size, hipStream_t stream)
  {
    ...
    if (!block_found) {
      ...
      TORCH_CHECK_WITH(HIPOutOfMemoryError, false,
        "HIP out of memory. Tried to allocate ", format_size(alloc_size),
        " (GPU ", device, "; ",
    ...
    TORCH_INTERNAL_ASSERT(params.err == hipSuccess &&
    ...
    c10::reportMemoryUsageToProfiler(
        block, block->size, c10::Device(c10::DeviceType::HIP, device));
  ...
  void free(Block* block)
  {
    ...
    c10::reportMemoryUsageToProfiler(
        block, -block->size, c10::Device(c10::DeviceType::HIP, block->device));
  ...
  void recordStream(Block* block, hip::HIPStream stream) {
    std::lock_guard<std::recursive_mutex> lock(mutex);
  ...
  void cacheInfo(size_t* total, size_t* largest)
  {
    ...
    if (*largest == 0) {  // make an initial guess if a zero *largest is passed in
      size_t tmp_bytes;
      hipMemGetInfo(largest,  // Use free memory as an optimistic initial guess of *largest
                     &tmp_bytes);
    }
 ...
 private:
  ...
  bool trigger_free_memory_callbacks(AllocParams& p) {
    ...
    for (const auto& name : FreeHipMemoryCallbacksRegistry()->Keys()) {
      freed_memory |=
        FreeHipMemoryCallbacksRegistry()->Create(name)->Execute();
    }
    return freed_memory;
  }
  ...
  bool alloc_block(AllocParams& p, bool isRetry) {
    ...
    if (set_fraction && total_allocated_memory + size > allowed_memory_maximum) {
      ...
      p.err = hipMalloc(&ptr, size);
      if (p.err != hipSuccess) {
        if (p.err == hipErrorMemoryAllocation) {
          // If this is the first attempt (!isRetry), we can forgive and clear HIP's
          ...
          //   In this case, we can also forgive and clear HIP's internal error state.
          hipGetLastError();
        } else {
  ...
  void free_blocks(BlockPool& blocks)
  {
    ...
    while (it != blocks.end()) {
      ...
      if (!block->prev && !block->next) {
        C10_HIP_CHECK(hipFree((void*)block->ptr));
  ...
  hipEvent_t create_event_internal() {
    hipEvent_t event;
    C10_HIP_CHECK(hipEventCreateWithFlags(&event, hipEventDisableTiming));
    return event;
  }

  void free_event_internal(hipEvent_t event) {
    C10_HIP_CHECK(hipEventDestroy(event));
  }

  void synchronize_and_free_events() {
    // Synchronize on outstanding events and then free associated blocks.

    for (auto& e : hip_events) {
      hipEvent_t event = e.first;
      Block* block = e.second;

      C10_HIP_CHECK(hipEventSynchronize(event));
  ...
  {
    int prev_device;
    C10_HIP_CHECK(hipGetDevice(&prev_device));

    stream_set streams(std::move(block->stream_uses));
    AT_ASSERT(block->stream_uses.empty());
    for (auto it = streams.begin(); it != streams.end(); ++it) {
      C10_HIP_CHECK(hipSetDevice(it->device_index()));

      hipEvent_t event = create_event_internal();
      C10_HIP_CHECK(hipEventRecord(event, it->stream()));

      block->event_count++;
      hip_events.emplace_back(event, block);
    }

    C10_HIP_CHECK(hipSetDevice(prev_device));
  }
...
class THCCachingAllocator {
 ...
 private:
  ...
  mutable std::mutex hip_free_mutex;
 ...
 public:
  ...
  std::mutex* getHipFreeMutex() const {
  ...
  void recordStream(const DataPtr& ptr, hip::HIPStream stream) {
...
bool forceUncachedAllocator() {
  static bool force_uncached =
      getenv("PYTORCH_NO_HIP_MEMORY_CACHING") != nullptr;
  return force_uncached;
}

static void uncached_delete(void* ptr) {
  C10_HIP_CHECK(hipFree(ptr));
}

// NB: I decided not to fold this into THCCachingAllocator, because the latter
// has a lot more methods and it wasn't altogether clear that they should
// actually be publicly exposed
struct HipCachingAllocator : public Allocator {
  DataPtr allocate(size_t size) const override {
    int device;
    C10_HIP_CHECK(hipGetDevice(&device));
    void* r = nullptr;
    if (forceUncachedAllocator()) {
      C10_HIP_CHECK(hipMalloc(&r, size));
      return {r, r, &uncached_delete, Device(DeviceType::HIP, device)};
    }
    if (size != 0) {
      caching_allocator.malloc(&r, device, size, hip::getCurrentHIPStream(device));
    }
    return {r, r, &raw_delete, Device(DeviceType::HIP, device)};
  }
...
HipCachingAllocator device_allocator;
...
{
  return caching_allocator.getHipFreeMutex();
}
...
//
// HIP IPC only allows sharing a big memory block associated with a hipIpcMemHandle_t
...
//
// ipcMemHandle_to_devptr maps a hipIpcMemHandle_t to a device pointer in the process
...
// will be used to reconstruct all storages in this HipMalloc allocation.
// And it will deleted in hipIpcCloseMemHandle when its reference count is 0.
//
namespace {
  std::mutex IpcMutex;
...
std::shared_ptr<void> getIpcDevPtr(std::string handle) {
  ...
  C10_HIP_CHECK(hipIpcOpenMemHandle(&dev, *ipc_handle, hipIpcMemLazyEnablePeerAccess));
  // devPtr has to be deleted in same device when created.
  int curr_device;
  C10_HIP_CHECK(hipGetDevice(&curr_device));
  auto sp = std::shared_ptr<void>(
      dev,
      [handle, curr_device](void *ptr) {
        hip::HIPGuard device_guard(curr_device);
        std::lock_guard<std::mutex> deleter_lock(IpcMutex);
        C10_HIP_CHECK(hipIpcCloseMemHandle(ptr));
...
void* raw_alloc(size_t nbytes) {
  ...
  C10_HIP_CHECK(hipGetDevice(&device));
  void* r = nullptr;
  caching_allocator.malloc(&r, device, nbytes, hip::getCurrentHIPStream(device));
  return r;
}

void* raw_alloc_with_stream(size_t nbytes, hipStream_t stream) {
  if (nbytes == 0) {
    return nullptr;
  }
  int device;
