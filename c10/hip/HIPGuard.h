...
struct OptionalHIPGuard {
  ...
  explicit OptionalHIPGuard(optional<Device> device_opt) : guard_(device_opt) {}

  /// Set the current HIP device to the passed device index, if it is not
  /// nullopt
  explicit OptionalHIPGuard(optional<DeviceIndex> device_index_opt) : guard_(device_index_opt) {}
  ...
  /// is not already initialized.  Errors if the given device is not a HIP device.
  void set_device(Device device) { guard_.set_device(device); }
  ...
  /// Restore the original HIP device, resetting this guard to uninitialized state.
...
struct HIPStreamGuard {
  ...
  /// Set the current HIP device to the device associated with the passed stream,
  /// and set the current HIP stream on that device to the passed stream.
  /// Errors if the Stream is not a HIP stream.
  ...
  /// Move is disallowed, as HIPStreamGuard does not have an uninitialized state,
  ...
  /// Returns the HIP stream that was set at the time the guard was constructed.
...
/// A variant of OptionalStreamGuard that is specialized for HIP.  See HIPGuard
/// for when you can use this.
struct OptionalHIPStreamGuard {
  ...
  explicit OptionalHIPStreamGuard(optional<Stream> stream_opt) : guard_(stream_opt) {}
  ...
  /// Returns the HIP stream that was set at the time the guard was most recently
  ...
  /// Restore the original HIP device and stream, resetting this guard to uninitialized state.
  void reset() { guard_.reset(); }
